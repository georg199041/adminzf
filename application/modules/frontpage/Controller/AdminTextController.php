...
class Frontpage_AdminTextController extends Core_Controller_Action
{	
	public function init()
	{
		$this->getHelper('layout')->setLayout('admin');
		$this->view->headTitle('Текст на главной');
	}
    ...
    public function editAction()
    {
    	...
    	$model = Core::getMapper('frontpage/text')->find($id);
    ...
    public function saveAction()
    {
    	if (($data = $this->getRequest()->getPost()) && !$this->getRequest()->getParam('cancel')) {
    		try {
    			$form = Core::getBlock('frontpage/admin-text/edit');
    			if (!$form->isValid($data)) {
    				Core::getSession('admin')->formHasErrors = true;
    				throw new Exception($this->__("Invalid form"));
    			}
    			
    			$model = Core::getMapper('frontpage/text')->create($form->getValues());
    			$model->save();
