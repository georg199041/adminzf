...
class Frontpage_AdminRecoverySystemController extends Core_Controller_Action
{	
    ...
    public function editAction()
    {
    	...
    	$model = Core::getMapper('frontpage/recovery-system')->find($id);
    ...
    public function saveAction()
    {
    	if (($data = $this->getRequest()->getPost()) && !$this->getRequest()->getParam('cancel')) {
    		try {
    			$form = Core::getBlock('frontpage/admin-recovery-system/edit');
    			if (!$form->isValid($data)) {
    				Core::getSession('admin')->formHasErrors = true;
    				throw new Exception($this->__("Invalid form"));
    			}
    			
    			$model = Core::getMapper('frontpage/recovery-system')->create($form->getValues());
    			$model->save();
