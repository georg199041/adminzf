...
class Videogallery_AdminVideosController extends Core_Controller_Action
{
	public function init()
	{
		$this->getHelper('layout')->setLayout('admin');
		$this->view->headTitle('Видеозаписи видеогалереи');
	}
	...
	public function editAction()
	{
		...
		$model = Core::getMapper('videogallery/videos')->find($id);
	...
	public function saveAction()
	{
		if (($data = $this->getRequest()->getPost()) && !$this->getRequest()->getParam('cancel')) {
			try {
				$form = Core::getBlock('videogallery/admin-videos/edit');
				if (!$form->isValid($data)) {
					Core::getSession('admin')->formHasErrors = true;
					throw new Exception($this->__('Invalid form'));
				}
				
				$model = Core::getMapper('videogallery/videos')->create($form->getValues());
				$model->save();
	...
	public function deleteAction()
	{
		...
		if (null === $ids) {
			Core::getBlock('application/admin/messeger')->addError($this->__('Не выбрана ни одна запись'));
		} else {
			try {
				...
				Core::getBlock('aplication/admin/messenger')->addSuccess($this->__($message . ' записей:') . ' ' . count($ids));
			} catch (Exception $e) {
	...
	public function moveAction()
	{
		...
		if (!is_array($ids)) {
			...
			try {
				...
				Core::getBlock('application/admin/messenger')->addErrors($this->__('Ошибка перемещения'));
			}
		}
		
		$this->getHelper('Regirector')->gotoRouteAndExit(Core::urlToOptions('*/*/index'), null, true);
	}
